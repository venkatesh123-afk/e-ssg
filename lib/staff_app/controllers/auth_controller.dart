  ...
  Future<void> login(String username, String password) async {
    try {
      ...
      if (isSuccess && token != null) {
        ...
        final resolvedRole =
            response["role"] ?? (loginType == 'student' ? 'Student' : 'Staff');
        AppStorage.saveUserRole(resolvedRole.toString());

        if (response['permissions'] != null) {
          AppStorage.savePermissions(
        ...
        if (loginType == 'student') {
          ...
          // 🚀 ROUTE BASED ON ROLE
          final roleStr = AppStorage.getUserRole()?.toLowerCase() ?? '';
